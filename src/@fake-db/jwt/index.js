...
mock.onPost('/jwt/login').reply(request => {
  const { phone, password } = JSON.parse(request.data)
  ...
  if (user) {
    ...
    error = {
      phone: ['phone or Password is Invalid']
    }
  }
...
mock.onPost('/jwt/register').reply(request => {
  if (request.data.length > 0) {
    const { fullName, email, password, username, phone } = JSON.parse(request.data)

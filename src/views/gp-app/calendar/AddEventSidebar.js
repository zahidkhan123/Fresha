...
const AddEventSidebar = props => {
  ...
  const [value, setValue] = useState([{ value: 'Business', label: 'Business', color: 'warning' }])
    ...
    if (!isObjEmpty(selectedEvent)) {
      ...
      const resolveLabel = () => {
        if (calendar.length) {
          ...
          return { value: 'Business', label: 'Business', color: 'dark' }
        }
      }
  ...
  const EventActions = () => {
    if (isObjEmpty(selectedEvent) || (!isObjEmpty(selectedEvent) && !selectedEvent.title.length)) {
      return (
        <Fragment>
          <Button.Ripple className='mr-1 button_slide slide_right ' type='submit' color='dark'>
           <span>Add </span> 
          </Button.Ripple>
